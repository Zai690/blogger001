...
router.get("/postupdate/:id", auth, function (req, res) {
  Post.findOne(
    ...
    function (err, rtn) {
      ...
      else res.redirect("/users/mypost");
    }
  );
});
...
router.post("/postupdate", auth, upload.single("photo"), function (req, res) {
  ...
  if (req.file) update.image = "/images/uploads" + req.file.fieldname;
  Post.findOneAndUpdate(
    {_id: req.body.id, author : req.session.user.id}, 
    { $set: update }, function (err, rtn) {
    if (err) throw err;
    console.log(rtn);
    ...
    (err, rtn) => {
      ...
      if (rtn != null) {
        Comment.find({ post: req.params.id })
          ...
          .exec((err2, rtn2) => {
            if (err2) throw err2;
            res.render("user/postdetail", { posts: rtn, comments: rtn2 });
          });
      } else {
        res.redirect("/users/mypost");
      }
    ...
    User.findById(req.session.user.id, (err, rtn) => {
      if (err) {
        ...
        let bloggerlist = rtn.favoriteB.filter(function (data) {
          return data.blogger.toString() != req.body.aid;
        });
        User.findByIdAndUpdate(
          req.session.user.id,
