...
router.get("/", function (req, res, next) {
  Post.aggregate([
    ...
    Comment.aggregate([
      ...
      if (err2) throw err2;
      console.log(rtn2[0]);
    res.render("index", { title: "Express", morelike: rtn, morecomment:rtn2 });
    }); 
  });
});
...
router.get("/postdetails/:id", function (req, res) {
  Post.findById(req.params.id)
    ...
    .exec((err, rtn) => {
      ...
      Comment.find({ post: req.params.id })
        ...
        .populate("author", "name")
        .select("created comment author reply commenter name")
        .exec((err2, rtn2) => {
          if (err2) throw err2;
          let reactStatus;
          let favStatus;
